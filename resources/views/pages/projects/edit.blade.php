    ...
    <div class="py-1">
        ...
        <template x-if="is_success">
            <div class="alert alert-success">
                Berhasil mengubah proyek, anda sedang diarahkan ke daftar project
            </div>
        </template>
    </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group mb-4">
                        ...
                        <input x-model="analysis_date" type="text" class="form-control" id="analysisDate">
                    </div>
                </div>
            </div>
...
<script>
    ...
    @if($data->policy_date != null)
        policyDate.setDate("{{$data->policy_date}}");
    @endif
    @if($data->analysis_date != null)
        analysisDate.setDate("{{$data->analysis_date}}");
    @endif
    ...
    function _alpineInit() {
        return {
            ...
            implementation_periods_labels: convertLabels(),
            is_error: false,
            is_success: false,
            is_loading: false,
            errors: [],
            ...
            async sendData() {
                ...
                this.errors = [];
                try {
                    let response = await axios.post("{{route('project.edit_action', $data->id)}}", {
                        "_token": "{{csrf_token()}}",
