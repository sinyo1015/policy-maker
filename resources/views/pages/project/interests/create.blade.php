...
<div x-data="__alpineInit()">
    <div class="card border-light shadow-sm components-section">
            ...
            <div class="py-1">
                ...
                <template x-if="is_success">
                    <div class="alert alert-success">
                        Berhasil menambahkan interest, anda sedang diarahkan ke daftar interest
                    </div>
                </template>
            </div>
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    <div class="form-group">
                        ...
                        <select x-model="interest_type" name="" id="" class="form-control">
                            <option selected>--Pilih Interest--</option>
                            <template x-for="(_interest, i) of interests">
                                <option :value="_interest.id" x-text="_interest.name"></option>
                            </template>
                        </select>
                    </div>
                </div>
        ...
        <div class="card-footer">
            <div class="float-end">
                <button @click="sendData" :disabled="is_loading" class="btn btn-success text-white"><i class="fa fa-save"></i> Tambah</button>
            </div>
        </div>
    </div>
</div>
    ...
    function __alpineInit() {
        return {
            ...
            async sendData() {
                ...
                this.errors = [];
                try {
                    let response = await axios.post("{{route('project_interests.create_action', $id)}}", {
                        "_token": "{{csrf_token()}}",
                        player : this.player,
